...
bool Train::validTime(int value) const
{
	...
	{
		int minutes = value % 100; //returns the two right digits
		if (minutes <= 59)
		{
			return true;
...
{
	finalize(); //assign trainName to a safe empty state
...
{
	if (validTime(departure) && validNoOfPassengers(noOfPassengers)) //checks if the departure time and passengerNum is valid
	{
...
bool Train::load(int& leftPassengers)
{
	...
	cout << "> ";
	cin >> noOfPassengers;
	totalPassengers = sumOfPassengers(noOfPassengers); //total number of passengers
